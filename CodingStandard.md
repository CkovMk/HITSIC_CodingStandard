...
3. 工程中的头文件（`*.h`、`*.hpp`）与源文件（`*.c`、`*.cpp`）推荐以相同文件名成对出现。除了`main.c/main.cpp`（用于保存主程序）、`isr.c`（用于保存中断服务程序）等专用源文件或软件模块中用于存放平台移植接口的`port.c(pp)`源文件外，其他源文件不建议独立出现。

4. 包含C++代码的头文件和源文件，必须以`*.hpp`、`*.cpp`作为文件扩展名。

5. 源文件（`*.c`、`*.cpp`）中应该仅包含（`#include`）与自身同名的头文件（`*.h`、`*.hpp`）。C语言文件禁止包含含有C++代码的文件。极少数情况下，如果某个头文件中仅包含定义时即被初始化的变量，则源文件中可以包含这类头文件，但必须保证该头文件**仅在此源文件中包含一次**。
